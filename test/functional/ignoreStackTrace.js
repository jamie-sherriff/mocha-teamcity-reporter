...
describe('Check TeamCity Output is correct with outer suite', function () {
	...
	function verifyResults() {
		...
		it('custom stacktrace is present', function () {
			...
			expect(rowToCheck).to.include('message=\'custom stacktrace replacement');
	...
	describe('specified as an env var', function () {
		before(function (done) {
			const opts = {
				env: Object.assign({
					MOCHA_TEAMCITY_STACK_TRACE_MESSAGE: 'custom stacktrace replacement'
				}, process.env)
			};
	...
	describe('specified with --reporter-options', function () {
		before(function (done) {
			execFile(internalMochaPath, [
				...
				'lib/teamcity',
				'--reporter-options',
				'ignoreStackTrace=custom stacktrace replacement'
			], (err, stdout, stderr) => {
				teamCityStdout = stdout;
				teamCityStderr = stderr;
				teamCityOutputArray = stdout.split('\n');
