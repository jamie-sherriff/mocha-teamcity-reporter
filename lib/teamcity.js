...
const TEST_FAILED_COMPARISON = `##teamcity[testFailed type='comparisonFailure' name='%s' message='%s' details='%s' captureStandardOutput='true' flowId='%s' expected='%' actual='%s']`;
...
function Teamcity(runner, options) {
	...
	runner.on('fail', function (test, err) {
		if(ignoreStackTrace) {
			...
			log(formatString(TEST_FAILED_COMPARISON, test.title, ignoreStackTrace, err.message, flowId, test.expectedResult, test.actualResult));
		} else{
