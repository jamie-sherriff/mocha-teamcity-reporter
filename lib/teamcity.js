...
const TEST_FAILED_COMPARISON = `##teamcity[testFailed type='comparisonFailure' name='%s' message='%s' details='%s' captureStandardOutput='true' expected='%' actual='%s' flowId='%s']`;
...
function Teamcity(runner, options) {
	...
	const ignoreStackTrace = reporterOptions.ignoreStackTrace || process.env['MOCHA_TEAMCITY_STACK_TRACE_MESSAGE'];
	const failComparison = reporterOptions.failComparison || process.env['MOCHA_TEAMCITY_FAIL_COMPARISON'];
	...
	runner.on('fail', function (test, err) {
		if(ignoreStackTrace) {
			log(formatString(TEST_FAILED, test.title, ignoreStackTrace, err.message, flowId));
		}else if(failComparison){
			log(formatString(TEST_FAILED_COMPARISON, test.title, ignoreStackTrace, err.message, test.expectedResult, test.actualResult, flowId));
		} else{
